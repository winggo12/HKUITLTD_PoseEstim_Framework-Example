...
class ArdhaVasisthasana: YogaBase{
    ...
    private func calculateScore(){
        ...
        let l_handfoot_v_dis = FeedbackUtilities.cal_vertical_dis(coor1: resultArray![5], coor2: resultArray![11])
        let r_handfoot_v_dis = FeedbackUtilities.cal_vertical_dis(coor1: resultArray![6], coor2: resultArray![12])
        
        var cb_s:UInt? = 0b1000000000000
        
        if ( l_handfoot_v_dis < r_handfoot_v_dis)
        {
            shoulder_score = FeedbackUtilities.left_shoulder(resultArray!, 60, 20, true)
            cb_s = ColorFeedbackUtilities.left_shoulder(score: shoulder_score)
        }
        else
        {
            shoulder_score = FeedbackUtilities.right_shoulder(resultArray!, 60, 20, true)
            cb_s = ColorFeedbackUtilities.right_shoulder(score: shoulder_score)
        }
        
        let left_arm_angle = FeedbackUtilities.getAngle(resultArray![6], resultArray![2], resultArray![12])
        let right_arm_angle = FeedbackUtilities.getAngle(resultArray![5], resultArray![1], resultArray![11])
        left_arm_score = FeedbackUtilities.angleToScore(left_arm_angle, 90, 10, true)
        right_arm_score = FeedbackUtilities.angleToScore(right_arm_angle, 90, 10, true)
        arm_score = 0.5*(left_arm_score + right_arm_score)
        
        left_arm_score = FeedbackUtilities.left_shoulder(resultArray!, 180.0, 10.0, true)
        right_arm_score = FeedbackUtilities.right_shoulder(resultArray!, 180.0, 10.0, true)
        arm_score = 0.5*(left_arm_score + right_arm_score)
